...
class TimeDBHelper {
  ...
  Future<Database> get database async {
    ...
    _db = openDatabase(
      ...
      join(await getDatabasesPath(), 'times.db'),
      // 데이터베이스가 처음 생성될 때, dog를 저장하기 위한 테이블을 생성합니다.
      onCreate: (db, version) {
        return db.execute(
          "CREATE TABLE times(id TEXT PRIMARY KEY, time TEXT, task TEXT)",
        );
      },
  ...
  Future<void> insertTime(PeriodicTime time) async {
  ...
  Future<List<PeriodicTime>> times() async {
    final db = await database;

    // 모든 Memo를 얻기 위해 테이블에 질의합니다.
    final List<Map<String, dynamic>> maps = await db.query('times');

    // List<Map<String, dynamic>를 List<PeriodicTime>으로 변환합니다.
  ...
  Future<void> updateTime(PeriodicTime time) async {
    final db = await database;
...
Future<void> saveTimeDB(String execTask) async {
  TimeDBHelper sd = TimeDBHelper();

  var fido = PeriodicTime(
    id: Str2Sha512(DateTime.now().toString()),
    time: DateTime.now().toString(),
    task: execTask,
  );

  await sd.insertTime(fido);

  print(await sd.times());
}
